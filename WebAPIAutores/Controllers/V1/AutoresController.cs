...
namespace WebAPIAutores.Controllers.V1
{
    ...
    //[Route("api/v1/autores")]// api/autores => ruta
    ...
    [ApiConventionType(typeof(DefaultApiConventions))]//esto nos sirve para retornar las posibles respuestas
    public class AutoresController : ControllerBase
    {
        ...
        [ServiceFilter(typeof(HATEOASAutorFilterAttribute))]//Esto es para acceder al filtro 
        
        public async Task<ActionResult<List<AutorDTO>>> Get([FromQuery]PaginacionDTO paginacionDTO) //incuimos el booleano por si queremos enlaces de HATEOAS
        {
            var queryable = context.Autores.AsQueryable();
            await HttpContext.InsertarParametrosPaginacionEnCabecera(queryable); //Con esto mandamos la cantidad de autores como un dato para el cliente
            ...
            var autores = await queryable.OrderBy(autor => autor.Nombre).Paginar(paginacionDTO).ToListAsync(); //utilizamos este metodo para paginar
        ...
        [HttpGet("{id:int}", Name = ("obtenerAutorv1"))]// obtencion a travez de una variable que se coloca entre llaves{}la restriccion es :int,
                                                      // Agregamos un nombre a la ruta para acceder a ella atraves del nombre 
        [AllowAnonymous]
        [ServiceFilter(typeof(HATEOASAutorFilterAttribute))]
        [ProducesResponseType(404)]//Con esto documentamos las respuestas que pueden retonar
        ...
        [HttpGet("{Nombre}",Name =("obtenerAutorPorNombrev1"))]// obtencion a travez de una variable que se coloca entre llaves{} 
        ...
        public async Task<ActionResult> Post([FromBody] AutorCreacionDTO autorCreacionDTO)
        {
            ...
            return CreatedAtRoute("obtenerAutorv1", new { id = autor.Id},autorDTO);//retornamos la ruta de un recurso ya creado
        }

        [HttpPut("{id:int}",Name = "actualizarAutorv1")]//para acturalizar un autor, se combina la ruta del controlador (Route) + el id 
        ...
        /// <returns></returns>
        [HttpDelete("{id:int}",Name = "borrarAutorv1")]//para eliminar un autor, se combina la ruta del controlador (Route) + el id (api/autores/id)
