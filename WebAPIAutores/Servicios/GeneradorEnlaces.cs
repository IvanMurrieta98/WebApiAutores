...
namespace WebAPIAutores.Servicios
{
    public class GeneradorEnlaces
    {
        ...
        private async Task<bool> EsAdmin()
        {
            ...
            var resultado = await authorizationService.AuthorizeAsync(httpContext.User, "esAdmin"); //para acceder al usuario necesitamos IHTTPContextAccessor
            return resultado.Succeeded;
        }

        

        public async Task GenerarEnlaces(AutorDTO autorDTO) //agregamos el autordto para ver los enlaces
        {
            var esAdmin = await EsAdmin();
            var Url = ConstruirURLHelper();
            autorDTO.Enlaces.Add(new DatoHATEOAS(
                enlace: Url.Link("obtenerAutor", new { id = autorDTO.Id }), //Para crear el URL necesitamos de IActionContextAccessor
                descripcion: "Self",
                metodo: "GET"));
